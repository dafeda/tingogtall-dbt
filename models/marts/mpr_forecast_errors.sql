...
, mpr_cpi AS (
    ...
    FROM {{ source('raw_nb', 'nb_mpr_indicators') }} mpr_cpi
    INNER JOIN published pub ON mpr_cpi.source = pub.raport
    WHERE mpr_cpi.indicator = 'cpi'
...
, mpr_cpi_ate AS (
    SELECT 
        ...
        pub.year_month_published
    FROM {{ source('raw_nb', 'nb_mpr_indicators') }} mpr_cpi_ate
    INNER JOIN published pub ON mpr_cpi_ate.source = pub.raport
    WHERE mpr_cpi_ate.indicator = 'cpi_ate'
...
, forecast_errors_policy_rate AS (
    SELECT
        ...
        - (CAST(SUBSTRING(mpr_policy_rate.year_month_published, 1, 4) AS INT) * 12 
        ...
        policy_rate_actuals.actual_value - mpr_policy_rate.forecast_value AS forecast_error,
        
        CASE 
            WHEN policy_rate_actuals.actual_value IS NULL THEN 'Not yet available'
            ELSE 'Available'
        END AS actual_status
    FROM mpr_policy_rate
    INNER JOIN policy_rate_actuals ON mpr_policy_rate.forecast_target = policy_rate_actuals.period_code
)
, error_bands_cpi AS (
    SELECT 
        ...
        PERCENTILE_CONT(0.10) WITHIN GROUP (ORDER BY forecast_error) AS p10_error,
        PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY forecast_error) AS p90_error,
...
, forecast_with_bands_cpi AS (
    ...
    FROM {{ source('raw_nb', 'nb_mpr_indicators') }} mpr_cpi
    INNER JOIN published pub ON mpr_cpi.source = pub.raport
    LEFT JOIN error_bands_cpi e ON (
        (CAST(SUBSTRING(mpr_cpi.year_month, 1, 4) AS INT) * 12 
         + CAST(SUBSTRING(mpr_cpi.year_month, 6, 2) AS INT))
        - (CAST(SUBSTRING(pub.year_month_published, 1, 4) AS INT) * 12 
         + CAST(SUBSTRING(pub.year_month_published, 6, 2) AS INT))
...
, forecast_with_bands_cpi_ate AS (
    ...
    FROM {{ source('raw_nb', 'nb_mpr_indicators') }} mpr_cpi_ate
    INNER JOIN published pub ON mpr_cpi_ate.source = pub.raport
    LEFT JOIN error_bands_cpi_ate e ON (
        (CAST(SUBSTRING(mpr_cpi_ate.year_month, 1, 4) AS INT) * 12 
         + CAST(SUBSTRING(mpr_cpi_ate.year_month, 6, 2) AS INT))
        - (CAST(SUBSTRING(pub.year_month_published, 1, 4) AS INT) * 12 
         + CAST(SUBSTRING(pub.year_month_published, 6, 2) AS INT))
...
, forecast_with_bands_policy_rate AS (
    ...
    FROM {{ source('raw_nb', 'nb_mpr_indicators') }} mpr
    INNER JOIN published pub ON mpr.source = pub.raport
    LEFT JOIN error_bands_policy_rate e ON (
        (CAST(SUBSTRING(mpr.year_month, 1, 4) AS INT) * 12 
         + CAST(SUBSTRING(mpr.year_month, 6, 2) AS INT))
        - (CAST(SUBSTRING(pub.year_month_published, 1, 4) AS INT) * 12 
         + CAST(SUBSTRING(pub.year_month_published, 6, 2) AS INT))
